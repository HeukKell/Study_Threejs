...
class App{
    ...
    _setupModel(){
        const geometry = new THREE.BoxGeometry(1,1,1);                          // 정육면체 모델 ( 가로, 세로, 깊이) 정의
        const material = new THREE.MeshPhongMaterial({color:0xff5555});         // 재질 ({색상})    정의
        
        const cube = new THREE.Mesh(geometry,material);                         // 모델 생성 (모델,재질)
